...
(function (window, document) {
	...
	Croppic = function (id, options) {
		...
		that.options = {
			...
			onAfterImgCrop: function(){ $('#profile_photo_hidden').val($('.croppedImg').attr('src'))},
	...
	Croppic.prototype = {
		...
		afterUpload: function(data){
            ...
            if (response.status == 'error') {
			  //  alert(response.message);
				showDialogMsg( 'Oops!', response.message, 3000,
					{
						buttons: [
							{
								text: "Ok",
		...
		reset:function(){
			...
			// Commented as appending image is not required as we have wrote code for
			// this appending image ourself.
			/*if( !$.isEmptyObject(that.croppedImg)){
